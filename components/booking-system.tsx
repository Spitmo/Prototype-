      ...
      <div className="text-center mb-12">
        <div className="relative inline-block p-4 mb-4">
          ...
          <div className="relative">
            <h2 className="text-3xl font-bold text-black">Book a Confidential Session</h2>
          </div>
        </div>
        <p className="text-muted-foreground text-lg">
          Choose from our qualified counselors and mental health professionals
        </p>
      </div>
      ...
      {!showBookingForm ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {counselors.map((counselor) => (
            <Card
              key={counselor.id}
              className={`cursor-pointer transition-all duration-300 hover:shadow-lg ${counselor.available ? "hover:scale-105" : "opacity-75"}`}
